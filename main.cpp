...
int main (int argc, char* argv[])
{
	...
	for (auto * t = list.getPreHead()->m_next; t != list.getPreHead(); t = t->m_next)
		cout << t->m_key << endl;

	auto* node = list.findFirst(5);
	list.remove(node);
...
//	list.remove(node);
	...
	node = list.findLastLessThan(3);
	list.remove(node);
//	list.remove(list.getPreHead());
//	NodeSkipList<int, int, 15>* nodeSkipList = new NodeSkipList<int, int, 15>(24,24);
//	list.remove(nodeSkipList);
